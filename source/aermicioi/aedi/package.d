...
Following methods are available to configure a struct/object's factory:
    $(OL
        $(LI construct - construct object using provided arguments)
        $(LI fact - use delegate/function to construct object)
        $(LI factoryMethod - use factory method pattern to create an object)
        $(LI set - set a field, call a method with passed arguments)
        $(LI autowire - construct/set with passed arguments)
        $(LI callback - call function/delegate to do some stuff on object)
    )
...
Following annotations can be used to configure a container:
    $(OL
        $(LI component -> Marks a struct or object as manageable by container. )
        $(LI constructor -> Marks constructor to be called. )
        $(LI setter -> Call, or set the annotated field. )
        $(LI autowired -> Depending what is annotated with autowired annotation, it will construct, set a field, or call a method, using FQN of argument type to search for dependencies in container).
        $(LI fact -> Use a function to construct object)
        $(LI callback -> Use a function to manipulate object somehow)
        $(LI @qualifier -> denotes how a component will be named in container )
    )

Currently the library provides 2 containers:
    $(OL
        $(LI singleton -> serves same object, during it's lifetime. )
        $(LI prototype -> serves new object, on each request. )
    )
    
Both of them support either way of configuring. 

$(BIG $(B Container for values: ))
...
--------------
Usage is pretty simple, call register, pass the value, and identity to it.
...
The GenericFactory(T) interface, splits the process of object construction in two steps:
    $(UL
        $(LI Object allocation, and construction)
...
$(UL
    $(LI canFactoryGenericFactory -> annotation will be used to create an implementation of GenericFactory(T) )
    $(LI canFactoryInstanceFactory -> annotation will be used to create an implementation of InstanceFactory(T) )
    $(LI canFactoryPropertyConfigurer -> annotation will be used to create an implementation of PropertyConfigurer(T) )
)
